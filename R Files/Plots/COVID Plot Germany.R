...
# Filter for Germany data
germany_data <- combined_dataset %>%
...
# Calculate xmax for lockdown shading
germany_data <- germany_data %>%
  arrange(month_date)  
germany_data$xmax <- c(germany_data$month_date[-1], max(germany_data$month_date))  

# Define scaling factor for y-axes
max_cases_per_100k_germany <- 15000
max_passengers_relative <- 100
scaling_factor_germany <- max_passengers_relative / max_cases_per_100k_germany
...
# Plot the data
ggplot(germany_data, aes(x = month_date)) +
  with_raster(
    geom_rect(
      data = germany_data %>% filter(travel_control_category != "0" & !is.na(travel_control_category)),
      aes(
        xmin = month_date,
        xmax = xmax,
        ymin = -Inf,
  ...
  geom_line(aes(y = new_cases_per_100k * scaling_factor_germany, color = "COVID-19 Cases per 100k"), linewidth = 2) +
  scale_x_date(
    date_labels = "%Y",
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ . / scaling_factor_germany, 
