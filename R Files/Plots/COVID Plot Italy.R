...
# Filter for Italy data
italy_data <- combined_dataset %>%
...
italy_data <- italy_data %>%
  arrange(month_date) 
italy_data$xmax <- c(italy_data$month_date[-1], max(italy_data$month_date))  

# Define scaling factor for y-axes
max_cases_per_100k_italy <- 15000
max_passengers_relative <- 100
scaling_factor_italy <- max_passengers_relative / max_cases_per_100k_italy
...
# Plot the data
ggplot(italy_data, aes(x = month_date)) +
  with_raster(
    geom_rect(
      data = italy_data %>% filter(travel_control_category != "0" & !is.na(travel_control_category)),
      aes(
        xmin = month_date,
        xmax = xmax,
        ymin = -Inf,
  ...
  geom_line(aes(y = new_cases_per_100k * scaling_factor_italy, color = "COVID-19 Cases per 100k"), linewidth = 2) +
  scale_x_date(
    date_labels = "%Y",
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ . / scaling_factor_italy, 
