# Shaded Plot Spain


# Load necessary libraries
library(ggplot2)
library(dplyr)
library(scales)

# Filter for Spain data
spain_data <- combined_dataset %>%
  filter(country == "Spain")

# Filter for 2020-2024
spain_data <- spain_data %>%
  filter(format(month_date, "%Y") >= 2020)

# Calculate xmax for lockdown shading
spain_data <- spain_data %>%
  arrange(month_date)  
spain_data$xmax <- c(spain_data$month_date[-1], max(spain_data$month_date))  

# Define scaling factor for y-axes
max_cases_per_100k_spain <- 15000
max_passengers_relative <- 100
scaling_factor_spain <- max_passengers_relative / max_cases_per_100k_spain

# Add shading categories for international travel controls
spain_data <- spain_data %>%
  mutate(
    travel_control_category = cut(
      international_travel_controls,
      breaks = c(-Inf, 1, 2, 3, 4, Inf), 
      labels = c("0", "1", "2", "3", "4"),
      right = FALSE                        
    )
  )

# Ensure the data is sorted and prepare xmax for shading
spain_data <- spain_data %>%
  arrange(month_date) %>%
  mutate(
    xmax = c(month_date[-1], month_date[n()])
)

# Plot the data
ggplot(spain_data, aes(x = month_date)) +
  with_raster(
    geom_rect(
      data = spain_data %>% filter(travel_control_category != "0" & !is.na(travel_control_category)),
      aes(
        xmin = month_date,
        xmax = xmax,
        ymin = -Inf,
        ymax = Inf,
        fill = travel_control_category
      ),
      color = NA,
      alpha = 0.7,
      inherit.aes = FALSE
    )
  ) +
  geom_line(aes(y = combined_passengers_relative_to_2019, color = "Relative Change in Passengers compared to 2019 (%)"), linewidth = 2) +
  geom_line(aes(y = new_cases_per_100k * scaling_factor_spain, color = "COVID-19 Cases per 100k"), linewidth = 2) +
  scale_x_date(
    date_labels = "%Y",
    breaks = as.Date(paste0(2020:2024, "-01-01")),
    name = NULL
  ) +
  scale_y_continuous(
    limits = c(-100, 100),
    breaks = c(-100, -75, -50, -25, 0, 25, 50, 75, 100),  
    sec.axis = sec_axis(~ . / scaling_factor_spain, 
                        breaks = seq(0, 15000, by = 5000),
                        labels = scales::label_number(accuracy = 1))
  ) +
  scale_color_manual(
    values = c(
      "Relative Change in Passengers compared to 2019 (%)" = "#1F77B4",
      "COVID-19 Cases per 100k" = "#D62728"
    )
  ) +
  scale_fill_manual(
    name = NULL,
    values = c(
      "1" = "#E1C1AC",  
      "2" = "#C8A47D",  
      "3" = "#A6754A",  
      "4" = "#6D4228"   
    )
  ) +
  theme_minimal() +
  theme(
    axis.title.y.left = element_blank(),                   
    axis.title.y.right = element_blank(),                  
    axis.text.y = element_text(size = 22),                 
    axis.text.x = element_text(size = 22),                
    legend.position = "none",                              
    plot.title = element_text(size = 20, hjust = 0.5),  
    text = element_text(family = "serif")
  )
